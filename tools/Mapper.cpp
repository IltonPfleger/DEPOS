...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < ehdr.e_phnum; i++) {
        if (phdrs[i].p_type == PT_LOAD) {
            if (phdrs[i].p_vaddr + phdrs[i].p_memsz > max_addr) max_addr = phdrs[i].p_vaddr + phdrs[i].p_memsz;
    ...
    for (int i = 0; i < ehdr.e_shnum; i++) {
        ...
        if (strcmp(name, ".text") == 0) {
            ...
            if (app.data.start == 0 || shdrs[i].sh_addr < app.data.start) app.data.start = shdrs[i].sh_addr;
            if (shdrs[i].sh_addr + shdrs[i].sh_size > app.data.end) app.data.end = shdrs[i].sh_addr + shdrs[i].sh_size;
        } else if (strcmp(name, ".bss") == 0 || strcmp(name, ".sbss") == 0) {
            if (app.bss.start == 0 || shdrs[i].sh_addr < app.bss.start) app.bss.start = shdrs[i].sh_addr;
            if (shdrs[i].sh_addr + shdrs[i].sh_size > app.bss.end) app.bss.end = shdrs[i].sh_addr + shdrs[i].sh_size;
