...
int main(int argc, char *argv[]) {
    using Chunk       = DEPOS::Chunk;
    using Application = DEPOS::BootInformation::Application;
    ...
    Application app = {};
    new (&static_cast<Chunk &>(app)) Chunk(app.text.start(), max_addr - app.text.start());

    if (text.initialized) new (&app.text) Chunk(text.start, text.size());
    if (data.initialized) new (&app.data) Chunk(data.start, data.size());
    if (rodata.initialized) new (&app.rodata) Chunk(rodata.start, rodata.size());
    if (bss.initialized) new (&app.bss) Chunk(bss.start, bss.size());
