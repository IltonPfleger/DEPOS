  ...
  public:
    STL_Verifier_Interest(Microsecond expiry, Microsecond period, Boolean_Filter *interested)
        ...
        if (expiry > period && period > 0)
            _verifier_thread = new Periodic_Thread(expiry, &evaluate, reinterpret_cast<void *>(this));
        else if (period > 0)
            _verifier_thread = new Periodic_Thread(period, &evaluate, reinterpret_cast<void *>(this));
    }
