...
class TSTP::Security : private SmartData, private Data_Observer<Buffer> {
  ...
  public:
    ...
    class Peer {
      public:
        ...
        bool valid_deploy(const Space &where, const Time &when) { return _valid.contains(where, when); }
    ...
    class Pending_Key {
      public:
        ...
        const Master_Secret &master_secret() {
            if (_master_secret_calculated) return _master_secret;
        ...
        friend Debug &operator<<(Debug &db, const Pending_Key &p) {
            db << "{msc=" << p._master_secret_calculated << ",c=" << p._creation << ",pk=" << p._public_key
               << ",ms=" << p._master_secret << ",el=" << &p._el << "}";
            return db;
        }
    ...
    class DH_Request : public Control {
      public:
        DH_Request(const Region &d, const Public_Key &k) : Control(DH_REQUEST), _destination(d), _public_key(k) {}
    ...
    class DH_Response : public Control {
      public:
        DH_Response(const Public_Key &k) : Control(DH_RESPONSE), _public_key(k) {}
    ...
    class Auth_Request : public Control {
      public:
        Auth_Request(const Auth &a, const OTP &o) : Control(AUTH_REQUEST), _auth(a), _otp(o) {}
    ...
    class Auth_Granted : public Control {
      public:
        Auth_Granted(const Region &d, const Auth &a) : Control(AUTH_GRANTED), _destination(d), _auth(a) {}
    ...
    class Report : public Control {
      public:
        ...
        friend Debug &operator<<(Debug &db, const Report &r) {
            //            db << reinterpret_cast<const Control &>(r) << ",u=" << r._unit << ",e=" << r._error << ",r=" <<
            //            r._epoch_request;
            return db;
        }
    } __attribute__((packed));
  ...
  public:
    ...
    static void add_peer(const unsigned char *peer_id, UInt32 id_len, const Region &valid_region) {
        ...
        //if (!_key_manager) _key_manager = new /*(SYSTEM)*/ ::Thread(&key_manager);
    }

    static Time deadline(const Time &origin) { return origin + Math::min(KEY_MANAGER_PERIOD, KEY_EXPIRY) / 2; }
