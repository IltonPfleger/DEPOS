...
{
public:
    Monitoring(Unit_Dev_Expiry::List *supported_SmartData, Microsecond period=0) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::MONITOR, 0, 100000), period) {
...
private:
    ...
    virtual void description() const {
        // db << "Monitor((u,d)={";
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
        //     db << "(u=" << it->object()->unit() << ",d=" << it->object()->dev() << ",origin=" << it->object()->origin_time() << ")";
        // }
        // db << "}";
    }
};
...
{
public:
    Environment_Min_Max_Temperature_Boolean_Filter(Unit_Dev_Expiry::List *supported_SmartData, UInt32 dev, Time expiry, Microsecond period=0) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::Temperature, dev, expiry), period) {
...
private:
    virtual bool evaluate() {
        db<SmartData>(TRC) << "Environment_Min_Max_Temperature_Boolean_Filter[SEU]::evaluate()::this=" << this << ",_inputs->size()=" << _inputs->size() << ",head=" << _inputs->head() << ",tail=" << _inputs->tail() << endl;
        bool plausibility = true;
        Microsecond t = 0;
        for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
            db<SmartData>(INF) << "Environment_Min_Max_Temperature_Boolean_Filter[SEU]::evaluate()::value=" << *(it->object()->value<float>()) << ",it=" << it << ",next=" << it->next() << endl;
            plausibility &= *(it->object()->value<float>()) > 183.15; // > 90 oC (record lowest temperature on earth is -89.2)
            plausibility &= *(it->object()->value<float>()) < 333.15; // < 60 oC (record highest temperature on earth is 56.7)
    ...
    virtual void description() const {
        ...
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
        //     db << "(u=" << it->object()->unit() << ",d=" << it->object()->dev() << ")=" << *(it->object()->value<float>()) << ")";
        // }
        // db << "}";
    }
};
...
{
public:
    Dummy_Speed_Boolean_Filter(Unit_Dev_Expiry::List *supported_SmartData, UInt32 dev, Time expiry) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::Speed, dev, expiry)) {
...
private:
    ...
    virtual void description() const {
        ...
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
        //     db << "(u=" << it->object()->unit() << ",d=" << it->object()->dev() << ")=" << *(it->object()->value<int>()) << ")";
        // }
        // db << "}";
    }
};
...
{
public:
    Dummy_Camera_Boolean_Filter(Unit_Dev_Expiry::List *supported_SmartData, UInt32 dev, Time expiry) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::RAW_BGR, dev, expiry)) {
...
private:
    ...
    virtual void description() const {
        ...
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
        //     db << "(u=" << it->object()->unit() << ",d=" << it->object()->dev() << ")=(image is too big to print?), showing expired() instead=" << it->object()->expired() << ")";
        // }
        // db << "}";
    }
};
...
{
public:
    Dummy_Direction_Boolean_Filter(Unit_Dev_Expiry::List *supported_SmartData, UInt32 dev, Time expiry) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::RAW_BGR, dev, expiry)) {
...
private:
    virtual bool evaluate() {
        ...
        Microsecond t = 0;
        for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
            plausibility &= *(it->object()->value<char>()) - '0' < 4 && *(it->object()->value<char>()) - '0' >= 0; // enum from 0 to 3
            if (it->object()->origin_time() > t) {
                t = Microsecond(it->object()->origin_time());
            }
        }
    ...
    virtual void description() const {
        ...
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
        //     db << "(u=" << it->object()->unit() << ",d=" << it->object()->dev() << ")=" << *(it->object()->value<char>()) << ")";
        // }
        // db << "}";
    }
};

template<bool THROTTLE>
class Dummy_Acceleration_Boolean_Filter : public Boolean_Filter, SmartData
{
private:
    static const int MAXIMUM_ACCELERATION           = 10; // ms2
    static const int MAXIMUM_BRAKE                  = 10; // ms2
public:
    Dummy_Acceleration_Boolean_Filter(Unit_Dev_Expiry::List *supported_SmartData, UInt32 dev, Time expiry) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::Acceleration, dev, expiry)) {
...
{
public:
    Dummy_Steer_Angle_Boolean_Filter(Unit_Dev_Expiry::List *supported_SmartData, UInt32 dev, Time expiry) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::Angle, dev, expiry)) {
...
{
public:
    Dummy_GPS_Boolean_Filter(Unit_Dev_Expiry::List *supported_SmartData, UInt32 dev, Time expiry) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::GPS3I, dev, expiry)) {
...
public:
    ...
    : maximum_acceleration(max_acc), maximum_brakage(max_brk), minimum_brakage(min_brk), maximum_lat_acceleration(max_lat_acc), minimum_lat_brakage(min_lat_brk) {}
...
public:
    RSS_Safe_Distance(Unit_Dev_Expiry::List *supported_SmartData, Road_Parameters *rp_ego, Road_Parameters *rp_other, Microsecond rho) 
    : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(SmartData::Unit::SAFETY_MODEL, 0, SmartData::Time(rho)), rho), _rp_ego(rp_ego), _rp_other(rp_other), _response_time(rho), _last_consumption(0) {}
    ~RSS_Safe_Distance() {}
    ...
    virtual bool evaluate() {
        ...
        for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) { // we still need to add road boundaries
            if (it->object()->origin_time() == 0)
                return false;
            db<SmartData>(INF) << "RSS:" << it->object()->unit_dev() << endl;

            if (it->object()->unit() == SmartData::Unit(SmartData::Unit::MOTION_VECTOR_LOCAL)) { // forces to use Unit operator==
        ...
        if (ego_reference) {
            ...
            if (_count == Verifiable_SmartData::ITERATIONS_MEASURE) {
                db<SmartData>(LOGGER) << "RSSlon&lat ==> SUM of " <<  Verifiable_SmartData::ITERATIONS_MEASURE << "=" << ((double)_sum)/1000. << ",avg=" << (((double)_sum)/1000.)/ Verifiable_SmartData::ITERATIONS_MEASURE << endl;
            }
            _count++;
    ...
    virtual void description() const {
        db<SmartData>(LOGGER) << "RSS_Boolean_Filter(={safe_lateral=" << _lateral_safe << ",safe_longitudinal=" << _longitudinal_safe << endl;
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++)
        //     it->object()->log_state();
    }
...
private:
    void check_object(Motion_Vector* ego, Motion_Vector* other) {
        ...
        db<SmartData>(TRC) << "LonDist:" << lon_dist << ",LatDist=" << lat_dist << ",heading=" << heading << ",otherisinfront=" << other_in_front << ",othertoright=" << other_to_right << ",ego_s=" << ego->_speed << ",other_s" << other_speed << endl;

        if (!other_in_front) { // i am in front -- no need for longitudinal verification
            if (lon_dist <= Traits<Object_Classes>::PASSENGER_CAR_LENGTH/2) { // if in danger laterally (e.g., they can hit laterally)
                if (other_to_right) {
                    dmin = minimum_lateral_distance(ego->_speed*Math::cos(other->_heading), other_speed*Math::cos(other->_heading), static_obj, true);
                    db<SmartData>(INF) << "dmin_lat=" << dmin << endl;
                    _lateral_safe &= lat_dist >= dmin*100+Traits<Object_Classes>::PASSENGER_CAR_WIDTH/2; //cm
                } else {
                    dmin = minimum_lateral_distance(other_speed*Math::cos(other->_heading), ego->_speed*Math::cos(other->_heading), static_obj, false);
                    db<SmartData>(INF) << "dmin_lat=" << dmin << endl;
                    _lateral_safe &= lat_dist >= dmin*100+Traits<Object_Classes>::PASSENGER_CAR_WIDTH/2; //cm
                }
            }
        } else {
            if (lat_dist <= Traits<Object_Classes>::PASSENGER_CAR_WIDTH/2) { // if in dange longitudinally (e.g., they are close laterally)
                dmin = minimum_frontal_distance(ego->_speed*Math::sin(other->_heading), other_speed*Math::sin(other->_heading), static_obj);
                db<SmartData>(INF) << "dmin_lon=" << dmin*100 << "+" << Traits<Object_Classes>::PASSENGER_CAR_LENGTH/2 << ",d" << lon_dist << endl;
    ...
    float minimum_frontal_distance(float vr, float vf, bool static_obj) {
        ...
        if (static_obj) {
            dmin = vr * rho + (_rp_ego->maximum_acceleration *rho * rho)/2 +
                    ((vr + rho * _rp_ego->maximum_acceleration)*(vr + rho * _rp_ego->maximum_acceleration))/(2*_rp_ego->minimum_brakage)
                    - 0;
        } else {
            dmin = vr * rho + (_rp_ego->maximum_acceleration *rho * rho)/2 +
                    ((vr + rho * _rp_ego->maximum_acceleration)*(vr + rho * _rp_ego->maximum_acceleration))/(2*_rp_ego->minimum_brakage)
    ...
    float minimum_lateral_distance(float v1, float v2, bool static_obj, bool ego_left=true /*, float l, float lout*/) {
        float dmin;
        ...
        if (static_obj) {
            ...
            dmin = (rho * (v1 + v1p) / 2) + 
                    ((v1p*v1p) / (2 * _rp_left->minimum_lat_brakage)) -
                    0;
        } else {
            float v1p = v1 + rho * _rp_left->maximum_lat_acceleration;
            ...
            dmin = (rho * (v1 + v1p) / 2) + 
                    ((v1p*v1p) / (2 * _rp_left->minimum_lat_brakage)) -
                    ((rho * (v2 + v2p) / 2) +
                    ((v2p*v2p) / (2 * _rp_right->minimum_lat_brakage)));
        }
