...
class NIC_Common {
  ...
  public:
    ...
    template <UInt32 LENGTH> class Address {
      public:
        enum Null { NULL = 0 };
      ...
      public:
        ...
        bool operator==(const Address &a) const {
            for (UInt32 i = 0; i < LENGTH; ++i) {
                if (_address[i] != a._address[i]) return false;
            }
            return true;
        }
        ...
        bool operator!=(const Address &a) const {
            for (UInt32 i = 0; i < LENGTH; ++i) {
                if (_address[i] != a._address[i]) return true;
            }
            return false;
        }
        ...
        UInt32 operator%(UInt32 i) const { return _address[LENGTH - 1] % i; }
    ...
    struct TSTP_Metadata {
        ...
        friend OStream &operator<<(OStream &db, const TSTP_Metadata &m) {
            db << "{rssi=" << m.rssi << ",period=" << m.period << ",sfdts=" << m.sfdts << ",id=" << m.id
               << ",offset=" << m.offset << ",destined_to_me=" << m.destined_to_me << ",downlink=" << m.downlink
               << ",deadline=" << m.deadline << ",my_distance=" << m.my_distance << ",sender_distance=" << m.sender_distance
               << ",is_new=" << m.is_new << ",is_microframe=" << m.is_microframe << ",relevant=" << m.relevant
               << ",trusted=" << m.trusted << ",freed=" << m.freed << ",hint=" << m.hint << "}";
            return db;
        }
    };
...
template <typename Family> class NIC : public Family, public Family::Observed {
  public:
    using Family::MTU;
