...
class UDP_Socket : public Observed
{
   ...
   public:
    ...
    int send(const unsigned char *data, UInt32 size) { return sendto(_socket[0], data, size, 0, (sockaddr *)&_group_socket, sizeof(_group_socket)); }
...
#define _SYS
   ...
   public:
    ...
    virtual int send(const Address &dst, const Protocol &prot, const void *data, UInt32 size)  // not used
    {
        ...
        // _aes.encrypt(reinterpret_cast<const unsigned char *>(data), reinterpret_cast<const unsigned char *>(GRP_KEY), encrypted);
        // mtx.unlock();
        ...
        // _aes.decrypt(reinterpret_cast<const unsigned char *>(encrypted), reinterpret_cast<const unsigned char *>(GRP_KEY), decrypted);
    ...
    {
        db<UDPNIC>(TRC) << "UDPNIC::alloc(s=" << address() << ",d=" << dst << ",p=" << hex << prot << dec << ",on=" << once << ",al=" << always
                        << ",ld=" << payload << ")" << endl;
    ...
    virtual int send(Buffer *buf)
    {
        ...
        if (encrypt_at_nic) {
            ...
            for (; i < buf->size() - remainders; i += KEY_SIZE)
                _aes.encrypt(buf->frame()->data<const unsigned char>() + i, reinterpret_cast<const unsigned char *>(GRP_KEY), encrypted + i);
    ...
    void update(typename UDP_Socket::Observed *obs)
    {
        ...
        if (encrypt_at_nic) {
            ...
            for (UInt32 i = 0; i < size; i += KEY_SIZE) {
                _aes.decrypt(reinterpret_cast<const unsigned char *>(data) + i, reinterpret_cast<const unsigned char *>(GRP_KEY), decrypted + i);
            }
            mtx.unlock();
        ...
        //_aes.decrypt(reinterpret_cast<const unsigned char *>(data), reinterpret_cast<const unsigned char *>(GRP_KEY), decrypted);

        buf->sfdts = ts;

        notify(prot, buf);  // if the UDPNIC is deleted when the receive thread called this function and the buffer was not freed yet, we have a memory leak.
    }
