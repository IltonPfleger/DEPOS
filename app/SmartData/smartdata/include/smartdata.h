...
namespace Space_Time {
    ...
    public:
        Short_Time() {};
        Short_Time(const Infinity32 &): _time(INFINITE32) {};
...
public:
    ...
    public:
        ...
        enum : UInt32 {
            ...
            MULTI_UNIT = 1 << 29,   // Value is an integral int stored in the 32 last significant bits of a 32-bit big-endian integer.
            MULTI_VALUE = 2 << 29,  // Value is an integral int stored in the 64 last significant bits of a 64-bit big-endian integer.
        ...
        template<UInt32 _MSD, UInt32 _TYPE, UInt32 _SUBTYPE, UInt32 _LEN>
        class Digital_Unit
        {
        public:
        ...
        enum Quantity : UInt32 {
            ...
            Road_Condition_Class   	= SI_Unit<DIR,     -4,     -4,     -4,     -4,     -4,     -4,     -4,     -3,     +0>::UNIT, // not an SI unit, an enum for road condition class.
        };
        ...
        enum Digital_Data: UInt32 {
            ...
            Dynamics                = Digital_Unit<1,     3,       1,   11*4+1>::UNIT,       // 1 vehicle dynamics (x, y, z, yaw, speed, steer, is-vehicle, bounding box(x,y,z),id)
            Dynamics_Array          = Digital_Unit<1,     3,       2,1+(11*4+1)*150>::UNIT,  // 1 char for counter of vehicles + up to 150 vehicle dynamics (x, y, z, yaw, speed, steer, is-vehicle, bounding box(x,y,z),id)
        ...
        struct Get
        {
            ...
            static const size_t DIGITAL_LEN = (UNIT & SID) == SI ? 0 :
                                            ...
                                            (UNIT >> 24) == (MOTION_VECTOR_LOCAL >> 24) ? ((UNIT & LEN) * sizeof(Motion_Vector))+1  :
                                            ...
                                            (UNIT >> 16) == (RAW_BGR >> 16)? ((UNIT & LEN) == 1 ? 1920*1080*3 : ( (UNIT & LEN) == 2 ? 1920*1200*3 : UNIT & LEN ) )             :
                                            // (x, y, z, i) * sizeof(float) * channels * points_per_second*delta_t (e.g., every 0.1 seconds) # maximum size, once lidars dropoff intensity 0
                                            (UNIT >> 16) == (PCD_MONOCROMATIC >> 16) ? ((UNIT & LEN) == 1 ? 4*4*32*(100000*0.1) : ( (UNIT & LEN) == 2 ? 16*2*12*(4*3+1) : UNIT & LEN ))             :
    ...
    public:
        ...
        static size_t digital_value_size(UInt32 unit) {
            ...
            if ((unit >> 24) == (MOTION_VECTOR_GLOBAL >> 24))
                // # elements * (1 byte to tell if ended + 3*(I32 Loc) + F32 speed + F32 Heading + F32 YawR. + F32 Accel) + 1 to tell if next is valid
                // plus ID and Confidence and class + I64 Time (to be removed after modifications in UNIT and MultiSmartData)
                return ((unit & LEN) * (1 + (3*4 + 4*4) + 8 + 4 + 4 + 8))+1;

            if ((unit >> 24) == (MOTION_VECTOR_LOCAL >> 24))
                // # elements * (1 byte to tell if ended + 3*(F32 Loc) + F32 speed + F32 Heading + F32 YawR. + F32 Accel) + 1 to tell if next is valid
                // plus ID and Confidence and class + I64 Time (to be removed after modifications in UNIT and MultiSmartData)
                return ((unit & LEN) * (1+ (3*4 + 4*4) + 8 + 4 + 4 + 8))+1;
            ...
            switch (unit >> 16) {
                ...
                case PCD_MONOCROMATIC >> 16:
                    return ((unit & LEN) == 1 ? 4*4*32*(100000*0.1) : ( (unit & LEN) == 2 ? 16*2*12*(4*3+1) : unit & LEN ) ); // Config 2: VLP16_POINTS_PER_PACKET = VLP16_POINTS_PER_BLOCK (16*2) * VLP16_BLOCKS_PER_PACKET (12) * LIDAR_Point (3 floats for x,y,z distance and 1 for char for intensity) 
                case PCD_RGB >> 16:
        ...
        friend OStream & operator<<(OStream & os, const Unit & u) {
            if(u & SI) {
                ...
                os << "{D:" << "M=" << ((u >> 29) & (0b11)) << ",t=" << ((u >> 24) & 0b11111) << ",subt=" << ((u >> 16) & (0xFF)) << ",l=" <<  (u & LEN);
            os << "}";
            return os;
        }
    ...
    public:
        ...
        _Header(const Unit & u, const Device_Id & d, const Type & t, const Mode & m, const Misc & mi = 0, const Signature & s=120)
        : _config((S & 0x03) << 6 | 0 << 5 | (t & 0x03) << 3 | (V1 & 0x07)), _mode(m), _misc(mi), _unit(u), _device(d), _signature(s) {}
        _Header(const Spacetime & o, const Unit & u, const Device_Id & d, const Type & t, const Mode & m, const Misc & mi = 0, const Signature & s=120)
        : _config((S & 0x03) << 6 | 0 << 5 | (t & 0x03) << 3 | (V1 & 0x07)), _mode(m), _misc(mi), _origin(o), _unit(u), _device(d), _signature(s) {}
        ...
        friend OStream & operator<<(OStream & os, const _Header & h) {
            ...
            case RESPONSE:
                os << "RES:";
                switch(h.operation()) {
                case ADVERTISE: os << "ADV:" << (((h.mode() & MODE_MASK) == COMMANDED) ? "R/W" : "R/O"); break;
            ...
            os << ",ver=" << h.version() - V0 << ",tr=" << h.time_request() << ",sc=" << h.scale() << ",lc=" << h._location_confidence << ",o=" << h._origin << ",u=" << h._unit << ",d=" << h._device << "sig=" << h._signature << ",lh=" << h._last_hop << "}";
            return os;
        }
    ...
    public:
        Response() {}
        Response(const Spacetime & origin, const Unit & unit, const Device_Id & device, const Mode & mode, const Uncertainty & uncertainty, const Time & expiry, const UInt16 fragment_index=0, const Signature s=120)
        : Header(origin, unit, device, RESPONSE, mode, uncertainty, s), _expiry(expiry), _fragment_index(fragment_index) {}
        ...
        template<typename T>
        T * data() { return reinterpret_cast<T *>(&_data); } // We need to discuss buf->frame() usage in security.cc

    private:
        Time _expiry;
        UInt16 _fragment_index; // is only != 0 when sizeof(Value) > MTU, up to 65535 fragments
    ...
    public:
        ...
        friend OStream & operator<<(OStream & os, const Command & m) { // printing m._data[0] generates a warning as _data has size 0
...
public:
    ...
    : _mode(mode), _origin(Locator::here(), Timekeeper::now()), _device(dev), _value(0), _uncertainty(UNCERTAINTY), _expiry(expiry), _signature(120),
     _transducer(new /*(SYSTEM)*/ Transducer(dev)), _predictor(predictive ? new /*(SYSTEM)*/ Predictor(typename Predictor::Configuration(), false) : 0), _thread(0), _link(this) {
        db<SmartData>(TRC) << "SmartData[R](d=" << dev << ",x=" << expiry << ",m=" << (((mode & MODE_MASK) == COMMANDED) ? "CMD" : ((mode & ADVERTISED) ? "ADV" : "PRI")) << ",Fragments=" << FRAGMENTS << ",MAX_MTU_Response=" << MAX_PAYLOAD_RESPONSE << ",last_frag_size=" << LAST_FRAGMENT_SIZE << ")=>" << this << endl;
        if(active)
...
private:
    void process(const Mode & op) {
        ...
        if(_mode & ADVERTISED) {
            if (FRAGMENTS > 0) {
                ...
                Response * response;
                if (op != RESPOND) {
                    buffer = Network::alloc(sizeof(Response), 0); // Advertise does not forward value, so there is no need to alloc sizeof(Value)
                    header = buffer->frame()->template data<Header>();
                    response = new (header) Response(_origin, UNIT, _device, (_mode | op), _uncertainty, _expiry, 0, _signature);
                ...
                Digital<sizeof(Value)> *value = reinterpret_cast<Digital<sizeof(Value)> *>(&_value); // avoid compilation errors on accessing value as a array, this is safe once FRAGMENTS > 0 is only true for Digital Data
                for (UInt32 i = 0; i < FRAGMENTS; i++)
                {
                    buffer = Network::alloc(sizeof(Response), MAX_PAYLOAD_RESPONSE);
                    header = buffer->frame()->template data<Header>();
                    response = new (header) Response(_origin, UNIT, _device, (_mode | op), _uncertainty, _expiry, i, _signature);
                    db<SmartData>(INF) << "SmartData[R]::process:preparing fragmentation:before copy << buffer->size()=" << buffer->size() << ",v[(i+1)*MAX_RSP_SIZE-1] is accessible?" << int(((*(value))[(i+1)*MAX_PAYLOAD_RESPONSE-1])) << endl;
                    response->value<Digital<MAX_PAYLOAD_RESPONSE>>(*reinterpret_cast<Digital<MAX_PAYLOAD_RESPONSE>*>(&((*value)[i*MAX_PAYLOAD_RESPONSE])));
                ...
                if (LAST_FRAGMENT_SIZE > 0) {
                    ...
                    header = buffer->frame()->template data<Header>();
                    response = new (header) Response(_origin, UNIT, _device, (_mode | op), _uncertainty, _expiry, FRAGMENTS, _signature);
                    response->value<Digital<LAST_FRAGMENT_SIZE>>(*reinterpret_cast<Digital<LAST_FRAGMENT_SIZE>*>(&((*value)[FRAGMENTS*MAX_PAYLOAD_RESPONSE])));
                    db<SmartData>(INF) << "SmartData[R]::process:last_frag["<< FRAGMENTS <<"]:msg=" << *response << endl;
                    Network::send(buffer);
                }
                ...
                if(op == RESPOND) {
                    ...
                    Response * response = new (header) Response(_origin, UNIT, _device, (_mode | op), _uncertainty, _expiry, 0, _signature); // This will overwrite TSTP Packet Id once sizeof(Response) > sizeof(Header)
                    ...
                    Buffer * buffer = Network::alloc(sizeof(Response), 0); // Advertise does not forward value, so there is no need to alloc sizeof(Value)
                    Header * header = buffer->frame()->template data<Header>();
                    Response * response = new (header) Response(_origin, UNIT, _device, (_mode | op), _uncertainty, _expiry, 0, _signature); // This will overwrite TSTP Packet Id once sizeof(Response) > sizeof(Header)
                    db<SmartData>(INF) << "SmartData[R]::process:msg=" << *response << endl;
                    Network::send(buffer);
    ...
    void update(typename Network::Observed * obs, Buffer * buffer) {
        Header * header = buffer->frame()->template data<Header>();
        db<SmartData>(TRC) << "SmartData[R]::update(obs=" << obs << ",buf=" << buffer << ",type=" << header->type() << ")" << endl;
        switch(header->type()) {
        case INTEREST: {
            Interest * interest = reinterpret_cast<Interest *>(header);
            db<SmartData>(TRC) << "SmartData[R]::update:msg=" << *interest << endl;
            // previous update was not verifying device nor containment in interest region... Unit verification was removed from TSTP, so it was added here
...
public:
    Interested_SmartData(const Region & region, Time expiry, Microsecond period = 0, Mode mode = SINGLE, Uncertainty uncertainty = ANY, Device_Id device = UNIQUE, Time_Offset timeout=0)
    : _mode(mode), _region(region), _device(device), _uncertainty(uncertainty), _expiry(expiry), _period(period), _predictor((predictive && (mode & PREDICTIVE)) ? new /*(SYSTEM)*/ Predictor : 0), _value(0), _fragments(0), _timeout(timeout), _started_update(0), _link(this) {
...
private:
    ...
    void update(typename Network::Observed * obs, Buffer * buffer) {
        ...
        case RESPONSE: {
            ...
            if((response->unit() == Unit(UNIT)) && _region.contains(response->origin()) && (response->device() == _device)) {
                if((response->operation()) == ADVERTISE)
                    process(ANNOUNCE);
                else {
                    _response = *response;
                    if (FRAGMENTS > 0) {
                        db<SmartData>(INF) << "SmartData[I]::update:receiving fragment=" << _response.fragment_index() << endl;
                        // if (_fragments == 0) {
                        //     _started_update = now();
                        // } else {
                        //     Time t_now = now();
                        //     if ( (t_now - _started_update) > _timeout) {
                        //         db<SmartData>(ERR) << "SmartData[I]::update:Timeout<" << _timeout << ">,start=" << _started_update << ",now=" << t_now << ",received_frags=" << _fragments << ",expected=" << (LAST_FRAGMENT_SIZE == 0 ? FRAGMENTS-1 : FRAGMENTS) << endl;
                        //         _started_update = t_now;
                        //         _fragments = 0;
                        //     }
                        // }
                        Digital<sizeof(Value)> *value = reinterpret_cast<Digital<sizeof(Value)>*>(&_value); // avoid compilation errors on accessing value as a array, this is safe once FRAGMENTS > 0 is only true for Digital Data
                        if ( (LAST_FRAGMENT_SIZE > 0 && _response.fragment_index() == FRAGMENTS)) { // last fragment (if LAST_FRAMENT_SIZE == 0, then last fragment has payload equal to MAX_PAYLOAD_RESPONSE)
                            if(_mode & CUMULATIVE)
                                *reinterpret_cast<Digital<LAST_FRAGMENT_SIZE>*>(&((*value)[FRAGMENTS*MAX_PAYLOAD_RESPONSE])) += response->template value<Digital<LAST_FRAGMENT_SIZE>>();
                            else
                                *reinterpret_cast<Digital<LAST_FRAGMENT_SIZE>*>(&((*value)[FRAGMENTS*MAX_PAYLOAD_RESPONSE])) = response->template value<Digital<LAST_FRAGMENT_SIZE>>();
                        } else {
                            if(_mode & CUMULATIVE)
                                *reinterpret_cast<Digital<MAX_PAYLOAD_RESPONSE>*>(&((*value)[_response.fragment_index()*MAX_PAYLOAD_RESPONSE])) += response->template value<Digital<MAX_PAYLOAD_RESPONSE>>();
                            else
                                *reinterpret_cast<Digital<MAX_PAYLOAD_RESPONSE>*>(&((*value)[_response.fragment_index()*MAX_PAYLOAD_RESPONSE])) = response->template value<Digital<MAX_PAYLOAD_RESPONSE>>();
                        }
                        db<SmartData>(INF) << "SmartData[I]::update:copied fragment!" << endl;
                         _fragments++;
                        if ((LAST_FRAGMENT_SIZE > 0 && _response.fragment_index() == FRAGMENTS) || (LAST_FRAGMENT_SIZE == 0 && _response.fragment_index() == FRAGMENTS-1)) { // unordered receive support + lost packet handling via timeout
                            db<SmartData>(LOGGER) << "SmartData[I]::update:received all fragments=" << _response.fragment_index() << endl;
                            _fragments = 0;
                            notify();
                        }
                    } else {
