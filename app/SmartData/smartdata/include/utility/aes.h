...
template <> class SWAES<16> : public AES_Common {
  private:
  ...
  private:
    ...
    void crypt(const unsigned char *data, const unsigned char *key, unsigned char *result, bool encrypt) {
        // db<Ciphers>(TRC) << "AES::" << (encrypt ? "en" : "de") << "crypt(data=" << data << ",key=" << key << ",result=" <<
        // result << endl; db<Ciphers>(INF) << "AES::" << (encrypt ? "en" : "de") << "crypt:data = {" << int(data[0]);
    ...
    void cbc_encrypt_buffer(unsigned char *output, const unsigned char *input, int length, const unsigned char *key,
                            unsigned char *iv);
    void cbc_decrypt_buffer(unsigned char *output, const unsigned char *input, int length, const unsigned char *key,
                            unsigned char *iv);
