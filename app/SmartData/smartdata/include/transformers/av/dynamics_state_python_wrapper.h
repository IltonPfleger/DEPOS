...
class Dynamics_State_Transformer: public Transducer<SmartData::Unit::MOTION_VECTOR_LOCAL|12<<16|1>, private Observer
{
    ...
    using Longitude_Proxy =         Interested_SmartData<SmartData::Unit::Wrap<(SmartData::Unit::Angle|SmartData::Unit::F32)>>;
    using Latitude_Proxy =          Interested_SmartData<SmartData::Unit::Wrap<(SmartData::Unit::Angle|SmartData::Unit::F32)>>;
    using Altitude_Proxy =          Interested_SmartData<SmartData::Unit::Wrap<(SmartData::Unit::Length|SmartData::Unit::F32)>>;
...
public:
    Dynamics_State_Transformer(const Device_Id & dev) : _value() {
        ...
        #endif
        _lonp   = new Longitude_Proxy(Longitude_Proxy::Region(0, 0, 0, 100, Longitude_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 7);
        _latp   = new Latitude_Proxy(Latitude_Proxy::Region(0, 0, 0, 100, Latitude_Proxy::now(),     INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 8);
        _altp   = new Altitude_Proxy(Altitude_Proxy::Region(0, 0, 0, 100, Altitude_Proxy::now(),     INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 9);

        _yp     = new Orientation_Proxy(Orientation_Proxy::Region(0, 0, 0, 100, Orientation_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 13);
        _pp     = new Orientation_Proxy(Orientation_Proxy::Region(0, 0, 0, 100, Orientation_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 14);
        _rp     = new Orientation_Proxy(Orientation_Proxy::Region(0, 0, 0, 100, Orientation_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 15);

        _yrp    = new Rotation_Rate_Proxy(Rotation_Rate_Proxy::Region(0, 0, 0, 100, Rotation_Rate_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 36);
        _prp    = new Rotation_Rate_Proxy(Rotation_Rate_Proxy::Region(0, 0, 0, 100, Rotation_Rate_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 37);
        _rrp    = new Rotation_Rate_Proxy(Rotation_Rate_Proxy::Region(0, 0, 0, 100, Rotation_Rate_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 38);

        _axp    = new Acceleration_Proxy(Acceleration_Proxy::Region(0, 0, 0, 100, Acceleration_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 0);
        _ayp    = new Acceleration_Proxy(Acceleration_Proxy::Region(0, 0, 0, 100, Acceleration_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 1);
        _azp    = new Acceleration_Proxy(Acceleration_Proxy::Region(0, 0, 0, 100, Acceleration_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 2);

        _wt_fl = new Wheel_Telemetry_Proxy(Wheel_Telemetry_Proxy::Region(0, 0, 0, 100, Wheel_Telemetry_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 60);
        _wt_fr = new Wheel_Telemetry_Proxy(Wheel_Telemetry_Proxy::Region(0, 0, 0, 100, Wheel_Telemetry_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 61);
        _wt_rl = new Wheel_Telemetry_Proxy(Wheel_Telemetry_Proxy::Region(0, 0, 0, 100, Wheel_Telemetry_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 62);
        _wt_rr = new Wheel_Telemetry_Proxy(Wheel_Telemetry_Proxy::Region(0, 0, 0, 100, Wheel_Telemetry_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, SmartData::ANY, 63);

        // inputs
        // speed -- this input may be available if some sensor for speed is implemented, otherwhise, this info is filled by the kalman filter
        if (!Traits<Project>::speed_derived) {
            _sxp = new Speed_Proxy(Speed_Proxy::Region(0, 0, 0, 100, Speed_Proxy::now(),    INFINITE), EXPIRY, 0, SmartData::SINGLE, 3);
        }
    ...
    void update(typename __UTIL::Observed *obs) {
        db<SmartData>(TRC) << "Dynamics_State updated!" 
                            ...
                            << " altp:" <<(_altp->when() > _last_consumption)
                            << endl;
        if (/*!_axp->expired() &&*/ _axp->when() > _last_consumption &&
            ...
            /*!_axp->expired() &&*/ _azp->when() > _last_consumption &&
             ...
             // we do not wait for an update on _strp as it is updated future in the flow with a smaller update rate, so, it is expected that every other SD is already updated
             ) {
            if (!Traits<Project>::speed_derived) {
                if (!(/*!_sxp->expired() &&*/ _sxp->when() > _last_consumption))
                    return;
            }
...
private:
    ...
    bool transform() {
            ...
            *reinterpret_cast<bool *>(&_value) = true;
            ...
            _value = *reinterpret_cast<const Value *>(data); // must return the same _value if nothing changed, if a value is updated, it should be returned in the respective position defined here
...
class Dynamics_State_Sensor : public Transducer<SmartData::Unit::MOTION_VECTOR_LOCAL|12<<16|1>, private UDP_Socket::Observer
{
...
public:
    Dynamics_State_Sensor(const Device_Id & dev): _value() {
