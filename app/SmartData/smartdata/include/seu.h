...
{
    SmartData::Time us = std::chrono::duration_cast<std::chrono::nanoseconds>(
...
class Unit_Dev_Expiry
{
   ...
   public:
    Unit_Dev_Expiry(SmartData::Unit u = 0, UInt32 d = 0, SmartData::Time expiry = 0) : _unit(u), _dev(d), _expiry(expiry), _link(this) {}
    ~Unit_Dev_Expiry() { db<SmartData>(TRC) << "UD delete" << endl; }

    friend bool operator==(const Unit_Dev_Expiry &ud1, const Unit_Dev_Expiry &ud2)
    {
        db<SmartData>(TRC) << "Unit_Dev_Expiry::operator!=(this=" << ud1._unit << "," << ud1._dev << "," << ud1._expiry << ";other=" << ud2._unit << ","
                           << ud2._dev << "," << ud2._expiry << ")" << endl;
...
public:
    static const UInt32 ITERATIONS_MEASURE = 1000;

   public:
    Verifiable_SmartData(UInt32 u, UInt32 d, Time e, Region r) : _ude(Unit_Dev_Expiry(u, d, e)), _region(r), _in_use(0), _link(this, _ude)
    {
        db<SmartData>(TRC) << "Verifiable_SmartData[SEU](u=" << u << ",d=" << d << ",e=" << e << ",r=" << r << ")" << endl;
        Network::attach(this);
        _value = new Data[_ude.unit().value_size()];
        if (_ude.unit().value_size() > 8)
            *reinterpret_cast<UInt32*>(_value) = 0x0;
        _sum = 0;
        _count = 0;
    }
    ...
    void add_sd_to_iot_queue()
    {
        ...
        if (((UInt32)(_ude.unit()) & SmartData::Unit::SID) == SmartData::Unit::SI) {
            switch (((UInt32)(_ude.unit()) & SmartData::Unit::NUM)) {
                case SmartData::Unit::I32:
                    value = std::to_string(*reinterpret_cast<Int32  *>(_value));
                    break;
                case SmartData::Unit::I64:
                    value = std::to_string(*reinterpret_cast<Int64 *>(_value));
                    break;
                case SmartData::Unit::F32:
                    value = std::to_string(*reinterpret_cast<float *>(_value));
                    break;
                default:
                    value = std::to_string(*reinterpret_cast<double *>(_value));
                    break;
            }
        }
    ...
    {  // TODO: make this a "<<" operator
        if (Traits<Build>::cloud) {
            ...
            if (((UInt32)(_ude.unit()) & SmartData::Unit::SID) == SmartData::Unit::SI) {
                db<SmartData>(LOGGER) << "(u=" << _ude.unit() << "=>" << (UInt32)_ude.unit() << ",d=" << _ude.dev() << ",t=" << origin_time()
                                  << ",sig=" << _response.signature() << ")={";
                switch (((UInt32)(_ude.unit()) & SmartData::Unit::NUM)) {
                    case SmartData::Unit::I32:
                        db<SmartData>(LOGGER) << *reinterpret_cast<Int32  *>(_value);
                        break;
                    case SmartData::Unit::I64:
                        db<SmartData>(LOGGER) << *reinterpret_cast<Int64 *>(_value);
                        break;
                    case SmartData::Unit::F32:
                        db<SmartData>(LOGGER) << *reinterpret_cast<float *>(_value);
                        break;
                    default:
                        db<SmartData>(LOGGER) << *reinterpret_cast<double *>(_value);
                        break;
                }
                ...
                db<SmartData>(LOGGER) << "(u=" << SmartData::Unit(SmartData::Unit::Speed|SmartData::Unit::F32) << "=>" 
                                  ...
                                  << ",d=" << _ude.dev() << ",t=" << origin_time() << ",sig=" << _response.signature() 
                                  << ")={" << wt->_wheel_speed << "}\n";
                db<SmartData>(LOGGER) << "(u=" << SmartData::Unit(SmartData::Unit::Force|SmartData::Unit::F32) << "=>" 
                                  ...
                                  << ",d=" << _ude.dev()+4 << ",t=" << origin_time() << ",sig=" << _response.signature() 
                                  << ")={" << wt->_wheel_lat_force << "}\n";
                db<SmartData>(LOGGER) << "(u=" << SmartData::Unit(SmartData::Unit::Force|SmartData::Unit::F32) << "=>" 
                                  << (UInt32) SmartData::Unit(SmartData::Unit::Force|SmartData::Unit::F32) 
            ...
            case RESPONSE: {
                ...
                if ((response->unit() == _ude.unit()) && _region.contains(response->origin()) && response->device() == _ude.dev()) {
                    if ((response->operation()) != ADVERTISE) {
                        ...
                        _response = *response;
                        if ((_ude.unit() >> 16) != (SmartData::Unit::RAW_BGR >> 16) && (_ude.unit() >> 16) != (SmartData::Unit::PCD_MONOCROMATIC >> 16))
                            memcpy(reinterpret_cast<unsigned char *>(_value), reinterpret_cast<const unsigned char *>(&(response->template value<Data>())),
                        ...
                        if (_count == ITERATIONS_MEASURE) {
                            db<SmartData>(LOGGER) << _ude.dev() << " ==> SUM of " << ITERATIONS_MEASURE << "=" << ((double)_sum)/1000. << ",avg=" << (((double)_sum)/1000.)/ITERATIONS_MEASURE << endl;
                        }
                        _count++;
                    }
                } else
...
protected:
    ...
    {
        // Speed XYZ (12B), Acceleration XYZ (12B), YPR Rate (12B), Location XYZ (12B), YPR (12B), Steer(4B), Battery State of
        // Charge (4B), Engine RPM (4B), Gear (4B)
...
public:
    Boolean_Filter(Unit_Dev_Expiry::List * supported, Unit_Dev_Expiry ude, Microsecond period=0) : _truth_value(true), _time(0), _supported_SmartData(supported), _ude(ude), _period(period), _link(this), _link2(this) {
        ...
        for(Unit_Dev_Expiry::List::Iterator it = _supported_SmartData->begin(); it != _supported_SmartData->end(); it++) {
            db<SmartData>(TRC) << "Boolean_Filter::supported_input(ud=" << it->object()->unit() << ",d=" << it->object()->dev() << ")" << endl;
        }
    }
    ...
    virtual ~Boolean_Filter() {
        ...
        delete _inputs;
        while (!_supported_SmartData->empty()) delete _supported_SmartData->remove_head()->object();
    ...
    virtual bool register_smartdata(Verifiable_SmartData *sd) {
        if (sd->unit_dev() == unit_dev() && _SmartData == 0) {
            _SmartData = sd;
            sd->attach(this);
            db<SmartData>(TRC) << "Boolean_Filter::register_smartdata: registered " << sd->unit_dev() << " in bf for " << unit_dev() << ",equal?" << (sd->unit_dev() == unit_dev()) << ",_SmartData=" << _SmartData << endl;
            return true;
        }
        db<SmartData>(TRC) << "Boolean_Filter::register_smartdata: attemped to register " << sd->unit_dev() << " in bf for " << unit_dev() << ",equal?" << (sd->unit_dev() == unit_dev()) << ",_SmartData=" << _SmartData << endl;
        return false;
    }

    virtual bool add_input(Verifiable_SmartData *sd) {
        Unit_Dev_Expiry ud = sd->unit_dev();
        db<SmartData>(TRC) << "Boolean_Filter()::add_input(ud=" << sd->unit() << ",d=" << sd->dev() << ",e=" << sd->expiry() << "),this=" << unit_dev() << endl;
...
public:
    virtual void update(Verifiable_SmartData *vsd) {
        ...
        db<SmartData>(TRC) << "Boolean_Filter[SEU]::notified():bf=" << this << "=>" << unit_dev() << ",by=" << vsd->unit_dev() << ",addr=" << vsd << endl;
...
class MU_Arrival : public Boolean_Filter, SmartData // basic boolean filter
{
public:
    MU_Arrival(SmartData::Unit unit, UInt32 dev, Time expiry, Microsecond period=0) : Boolean_Filter( new Unit_Dev_Expiry::List(), Unit_Dev_Expiry(unit, dev, expiry), period) {
...
private:
    ...
    virtual void description() const {
        ...
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
        //     db << "(u=" << it->object()->unit() << ",d=" << it->object()->dev() << ")= (t=" << it->object()->origin_time() << ", expired?=" << it->object()->expired() <<")";
        // }
        // db << "}";
    }
};

class MU_Arrival_Dep : public Boolean_Filter, SmartData // basic boolean filter
{
public:
    MU_Arrival_Dep(Unit_Dev_Expiry::List *supported_SmartData, SmartData::Unit unit, UInt32 dev, Time expiry, Microsecond period=0) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(unit, dev, expiry), period), _last_update(0) {
...
private:
    ...
    virtual void description() const {
        // if (_SmartData != 0)
        //     db << "MU_Arrival((u=" << unit_dev().unit() << ",d=" << unit_dev().dev() << ",expired?=" << _SmartData->expired() <<")={";
        // else
        //     db << "MU_Arrival((u=" << unit_dev().unit() << ",d=" << unit_dev().dev() << ",expired?=1)={";
        // for(Verifiable_SmartData::List::Iterator it = _inputs->begin(); it != _inputs->end(); it++) {
        //     db << "(u=" << it->object()->unit() << ",d=" << it->object()->dev() << ")= (t=" << it->object()->origin_time() << ", expired?=" << it->object()->expired() <<")";
...
class MU_Expired : public Boolean_Filter, SmartData // basic boolean filter
{
public:
    MU_Expired(Unit_Dev_Expiry::List *supported_SmartData, SmartData::Unit unit, UInt32 dev, Time expiry, Microsecond period=0) : Boolean_Filter(supported_SmartData, Unit_Dev_Expiry(unit, dev, expiry), period) {
...
public:
    STL_Verifier_Interest(Microsecond expiry, Microsecond period, Boolean_Filter* interested) : _period(period), _extended_stl_rule(0), _verifier_thread(0), _interested(interested), _interest(0), _link(this) {
        if (period <= expiry) {
        ...
        if (expiry > period && period > 0)
            _verifier_thread = new Periodic_Thread(expiry, 0, &evaluate, reinterpret_cast<void *>(this));
        else if (period > 0)
            _verifier_thread = new Periodic_Thread(period, 0, &evaluate, reinterpret_cast<void *>(this));
    }

    virtual ~STL_Verifier_Interest() { if (_verifier_thread != 0) delete _verifier_thread; delete _extended_stl_rule; _interested->detach(this); if (_interest != 0) _interest->detach(this); }

    void update(Boolean_Filter * bf) {
        db<SmartData>(TRC) << "STL_Verifier_Interest[SEU]::notified()(bf=" << bf << "=" << bf->unit_dev() << ",interested=" << _interested << "=" << _interested->unit_dev() << ",interest=" << _interest->unit_dev() << ",this=" << this << ")" << endl;
        _start = true;
        bool s = bf->result();
...
public:
    SEU_SmartData() : _safe(true) {
        ...
        _start = false;
        _period = 0;
        _thread = new Periodic_Thread(100000,0,&hyperperiod_verification, reinterpret_cast<void*>(this));
    }

    ~SEU_SmartData() {
        db<SmartData>(TRC) << "~SmartData[SEU](this=" << reinterpret_cast<void*>(this) << ")" << endl;
        Network::detach(this); // Rank ALL for Data Observers
        while (!_boolean_filters->empty()) delete _boolean_filters->remove_head()->object();
        delete _boolean_filters;
        while (!_stl_verifiers->empty()) delete _stl_verifiers->remove_head()->object();
    ...
    static void* hyperperiod_verification(void *p) {
        ...
        while(!d->_start) { Thread::yield(); }
        //usleep(d->_period);
        while(true) {
            Periodic_Thread::wait_next();
            for(STL_Verifier_Interest::List::Iterator it = d->_stl_verifiers->begin(); it != d->_stl_verifiers->end(); it++) {
                if (it->object()->_count == Verifiable_SmartData::ITERATIONS_MEASURE)
                    db<SmartData>(LOGGER) << "STL_Verifier_Interest[SEU](from=" << it->object()->_interested->unit_dev() << ",to" << it->object()->_interest->unit_dev() << ")"" ==> SUM of #Iterations(" << Verifiable_SmartData::ITERATIONS_MEASURE << ")=" << ((double)it->object()->_sum)/1000. << ",avg=" << (((double)it->object()->_sum)/1000.)/Verifiable_SmartData::ITERATIONS_MEASURE << ",avg_trace_len=" << (((double)it->object()->_sizes))/Verifiable_SmartData::ITERATIONS_MEASURE << endl;
            }
            ...
            //     }
            //     db<SmartData>(LOGGER) << "SmartData[SEU]::update(i from=" << it->object()->_interested->unit_dev() << ",to" << it->object()->_interest->unit_dev() << ")" << endl;
            //     if (it->object()->result() == false) {
            //         db<SmartData>(ERR) << "SYSTEM IS UNSAFE! Description:" << endl;
            //         db<SmartData>(ERR) << "\tInterest:" << it->object()->_interest->unit_dev() << ",last sample: time=" << it->object()->_interest->when() << ",mu=" << it->object()->_interest->result() << endl;
            //         db<SmartData>(ERR) << "\tInterested:" << it->object()->_interested->unit_dev() << ",last sample: time=" << it->object()->_interested->when() << ",mu=" << it->object()->_interested->result() << endl;
            //         db<SmartData>(ERR) << "\tNow:" << SEU_SmartData::now() << ",period=" << it->object()->_period << endl;
            //     };
            // }
        }
    }
    ...
    void print_stats() {
        db<SmartData>(LOGGER) << "SmartData[SEU]::finish!" << endl;
    }
...
private:
    ...
    void update(typename Network::Observed * obs, Buffer * buffer) {
        ...
        case INTEREST: {
            ...
            //     for(STL_Verifier_Interest::List::Iterator it = _stl_verifiers->begin(); it != _stl_verifiers->end(); it++) {
            //         removed = true;
            ...
            Verifiable_SmartData *vsd = new Verifiable_SmartData(interest->unit(), interest->device(), interest->expiry(), interest->region());
            ...
            for(Boolean_Filter::List::Iterator it = _boolean_filters->begin(); it != _boolean_filters->end(); it++) {
                ...
                if (used) {
                    if (it->object()->unit_dev() == Unit_Dev_Expiry(SmartData::Unit::MONITOR, 0, 100000)) {// ignore monitoring
                        ...
                        db<SmartData>(INF) << "SmartData[SEU]::used! who=" << it->object()->unit_dev() << ",bf_r=" << it->object() << ",interested in=" << vsd->unit_dev() << bf_left << endl;
                        rule = new STL_Verifier_Interest(Microsecond(interest->expiry()), it->object()->period(), it->object()); // it->object()->period()
                        db<SmartData>(INF) << "SmartData[SEU]::STL verifier created for ude=" << it->object()->unit_dev() << endl;
            ...
            if (!vsd->in_use()) {
                db<SmartData>(INF) << "SmartData[SEU]::before ignoring this one -- for some reason it was not used!" << endl;
                vsd->detach(bf_left);
                delete bf_left;
        ...
        case RESPONSE: {
            ...
            if (Traits<Build>::cloud) {
                    ...
                    #endif
                    // There is no need, if we can map all interest relations we already cover all relevant smartdata being
                    // generated Is there a need to check data that is not of interest to someone? Is it even useful to the
                    // system?
                }
    ...
    void update(STL_Verifier_Interest::Observed* obs) {
        ...
        db<SmartData>(TRC) << "SmartData[SEU]::update(from=" << i->_interested->unit_dev() << ",to" << i->_interest->unit_dev() << ")" << endl;

        if (i->result() == false) {
            db<SmartData>(ERR) << "SYSTEM IS UNSAFE! Description:" << endl;
            i->_interested->description();
            db<SmartData>(ERR) << "\tInterest:" << i->_interest->unit_dev() << ",last sample: time=" << i->_interest->_time << ",mu=" << i->_interest->_truth_value << ",elapsed=" << i->_interest->_elapsed << endl;
            db<SmartData>(ERR) << "\tInterested:" << i->_interested->unit_dev() << ",last sample: time=" << i->_interested->_time << ",mu=" << i->_interested->_truth_value << ",elapsed=" << i->_interested->_elapsed << endl;
...
void* STL_Verifier_Interest::evaluate(void * p) {
    ...
    while(!i->_start) { Thread::yield(); }
    Periodic_Thread::wait_next((TSC::time_stamp()+(8*i->_period)/10)); // add 80% of period
