...
template <> struct Traits<Machine> : public Traits<Machine_Common> {
    ...
    static const unsigned int MEM_TOP =
        0x10000000; // 256 MB (MAX for 32-bit is 0x70000000 / 1792 MB)
...
template <> struct Traits<System> : public Traits<Build> {
    ...
    static const bool multithread =
        (Traits<Build>::CPUS > 1) || (Traits<Application>::MAX_THREADS > 1);
...
template <> struct Traits<TSTP> : public Traits<Network> {
    ...
    static constexpr unsigned int NICS[] = {
        0}; // relative to NIC_Family (i.e. Traits<Ethernet>::DEVICES[NICS[i]]
