...
void TSTP::Router::update(Data_Observed<Buffer> *obs, Buffer *buf) {
    ...
    if (buf->is_microframe) {
        ...
        if (!buf->relevant) buf->relevant = forwarder && (buf->my_distance < buf->sender_distance);
    } else {
        ...
        if ((header->type() == CONTROL) && (header->subtype() == KEEP_ALIVE)) {
            ...
            buf->destined_to_me =
                (((header->origin().space == here()) && ((dest.center() == Region::Center(0, 0, 0)) && (dest.radius() == 0)) &&
                  (dest.contains(here(), dest.t0))) ||
                 ((header->origin().space != here()) && (dest.contains(here(), dest.t0))));
            if (buf->destined_to_me) db<TSTP>(INF) << "TSTP::Router::update:packet is for me" << endl;

            if (forward(buf)) {
                // Forward/acknowledge the packet
                if (buf->destined_to_me) return;
                ...
                // Copy frame contents
                memcpy(reinterpret_cast<unsigned char *>(send_buf->frame()), reinterpret_cast<unsigned char *>(buf->frame()),
                       buf->size());
...
void TSTP::Router::marshal(Buffer *buf) {
    ...
    buf->destined_to_me =
        (((buf->frame()->data<Header>()->origin().space == here()) &&
          ((dest.center() == Region::Center(0, 0, 0)) && (dest.radius() == 0)) && (dest.contains(here(), dest.t0))) ||
...
TSTP::Region TSTP::Router::destination(Buffer *buf) {
    ...
    case CONTROL:
        ...
        case DH_REQUEST: {
            Time t0 = header->origin().time;
            Time t1 = Security::deadline(t0);
            return Region(buf->frame()->data<Security::DH_Request>()->destination().center(),
                          buf->frame()->data<Security::DH_Request>()->destination().radius(), t0, t1);
        }
        case AUTH_GRANTED: {
            Time t0 = header->origin().time;
            Time t1 = Security::deadline(t0);
            return Region(buf->frame()->data<Security::Auth_Granted>()->destination().center(),
                          buf->frame()->data<Security::Auth_Granted>()->destination().radius(), t0, t1);
        }
        case REPORT: {
        ...
        case KEEP_ALIVE: {
            while (true) {
                Space fake(here().x() + (Random::random() % (RANGE / 3)), here().y() + (Random::random() % (RANGE / 3)),
                           (here().z() + Random::random() % (RANGE / 3)));
                if (fake != here()) return Region(fake, 0, 0, -1); // should never be destined_to_me
            }
        } break;
        case EPOCH: {
