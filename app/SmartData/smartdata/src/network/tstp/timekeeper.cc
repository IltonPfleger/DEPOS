...
void TSTP::Timekeeper::update(Data_Observed<Buffer> *obs, Buffer *buf) {
    ...
    if (buf->is_microframe) {
        if (!synchronized()) buf->relevant = true;
    } else {
        buf->deadline = Microsecond(Router::destination(buf).t1);
        bool closer_to_sink = buf->downlink ? ((here() - sink()) < (header->last_hop().space - sink()))
        ...
        if (synchronized()) {
            ...
            if (!closer_to_sink) {
                ...
                db<TSTP>(TRC) << "TSTP::Timekeeper::update:adjusted timer offset by " << _skew << ",t0=" << t0 << ",t1=" << t1
                              << endl;
                db<TSTP>(INF) << "TSTP::Timekeeper::update:the time is now " << now() << " us since EPOCH (" << _reference
                              << ")" << endl;
            }
        }
    }
...
void TSTP::Timekeeper::marshal(Buffer *buf) {
    ...
    if ((header->type() == CONTROL) && (header->subtype() == KEEP_ALIVE))
        ...
        buf->deadline =
            Microsecond(Router::destination(buf).t1); // deadline must be set after origin time for Security messages
}
