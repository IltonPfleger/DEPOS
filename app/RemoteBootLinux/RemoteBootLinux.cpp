            ...
            if (token == FDT_BEGIN_NODE) {
                ...
                if (is_target_node && strcmp(prop_name, property) == 0) {
                    if (prop_len == size) {
                        ...
                        // Não damos return true aqui imediatamente para manter o ponteiro 'current' íntegro,
                        // ou garantimos que ele pule o valor antes de retornar.
                ...
                // --- O ERRO ESTAVA AQUI ---
                // Você precisa pular o conteúdo da propriedade (alinhado a 4 bytes)
                // mesmo que não seja a propriedade que você quer editar.
                current += (prop_len + 3) & ~3;
...
int main() {
    ...
    size_t initramfs_size = tftp.request("initramfs.cpio", initramfs, LinuxMemorySize - kernel_size - dtb_size);
