...
void Memory::init() {
  ...
  *s_allocator = Allocator();
  ...
  for (unsigned long c = RamEnd - PageSize; c + PageSize > RamStart;
       c -= PageSize) {
    if (c + PageSize >= __kmm.start && c < __kmm.end)
      continue;
    if (c + PageSize >= __mm.start && c < __mm.end)
      continue;
    if (c + PageSize >= __bmm.start && c < __bmm.end)
      continue;
...
uintptr_t Memory::virt2phys(uintptr_t chunk) {
  if constexpr (Traits<Kernel>::Multitask)
    return chunk - Traits<MemoryMap>::VirtualRamStart +
           Traits<MemoryMap>::PhysicalRamStart;
  else
    return chunk;
}
