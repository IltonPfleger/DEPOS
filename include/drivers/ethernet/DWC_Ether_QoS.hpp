  ...
  public:
    static void init() {
        ...
        if (speed() == 1000) {
            ...
            MDIO::clear45(Phy, SYNCE_CFG, 0x7 << 1);
        }
...
template <typename MyTraits> class DWC_Ether_QoS_DMA : public Driver {
    typedef Meta::GetFromTypeList<Traits<DEPOS::CacheController>::Devices, 0>::Result CacheController;
  ...
  public:
    DWC_Ether_QoS_DMA()
        ...
        CacheController::flush(m_tx_descriptors, Number * sizeof(Descriptor));
    ...
    int send(const void *data, size_t length) {
        CacheController::flush(data, length);

        unsigned int head;
        unsigned int next;
        while (true) {
            head = CPU::Atomic::load(m_tx_head);
            next = (head + 1) % Number;
            if ((m_tx_descriptors[head].des3 & Descriptor::OWN) || next == m_tx_tail) {
                Alarm::udelay(1);
                continue;
            }
            if (CPU::Atomic::cas(m_tx_head, head, next)) break;
        }

        Descriptor &d = m_tx_descriptors[head];
        ...
        Reg32(Address, CH0_TX_TAIL_POINTER) = reinterpret_cast<uintptr_t>(m_tx_descriptors + head);

        // while (true) {
        //     Cache::invalidate(&d, sizeof(Descriptor));
        //     if (!(d.des3 & Descriptor::OWN)) {
        CPU::Atomic::finc(m_tx_tail);
    ...
    Buffer *receive() {
        ...
        CacheController::flush(buffer, sizeof(Buffer));
  ...
  private:
    ...
    static constexpr size_t MTU        = 1514;
...
template <typename Tag> class DWC_Ether_QoS final : public NIC {
  ...
  public:
    static constexpr size_t MaximumFrameLength = MyTraits::MTU + sizeof(PhysicalLayer::Header);
