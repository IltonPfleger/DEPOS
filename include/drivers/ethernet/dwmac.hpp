...
class DMA {
  ...
  public:
    static void reset() {
        TraceIn();
        Reg32(gmac_base, MODE) |= MODE_SOFTWARE_RESET;
        while (Reg32(gmac_base, MODE) & MODE_SOFTWARE_RESET)
            ;
        TraceOut();
    }
    static void init() {
        TraceIn();
        descriptors();
        Reg32(gmac_base, SYSBUS_MODE) |= SYSBUS_MODE_EAME;
        Reg32(gmac_base, CH0_TX_CONTROL) |= CH0_TX_CONTROL_ENABLE;
        Reg32(gmac_base, CH0_RX_CONTROL) |= CH0_RX_CONTROL_ENABLE;
        TraceOut();
    }
    ...
    static void send(unsigned char *frame, unsigned int length) {
        ...
        Cache::flush(&descriptor, sizeof(Descriptor));
        Reg32(gmac_base, CH0_TX_DESCRIPTORS_LIST_TAIL_POINTER) = reinterpret_cast<unsigned long>(m_tx_descriptors + 1);
