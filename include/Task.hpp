...
class Task {
  public:
    ...
    Task() : as(new(Heap::SYSTEM) AddressSpace()) {
        ...
        // reinterpret_cast<uintptr_t>(__mm.kernel.start); uintptr_t KernelEnd
        // = reinterpret_cast<uintptr_t>(__mm.kernel.end); Segment k =
        // Segment(__mm.kernel.start, KernelEnd - KernelStart); for (int i = 0;
    ...
    //       reinterpret_cast<uintptr_t>(__KERNEL_START__); uintptr_t KERNEL_END
    //       = reinterpret_cast<uintptr_t>(__KERNEL_END__); KERNEL_END =
    //       (KERNEL_END + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1); for (uintptr_t pa
    //       = KERNEL_START; pa < KERNEL_END; pa += PAGE_SIZE) as->map(pa, pa,
