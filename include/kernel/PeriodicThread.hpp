...
class PeriodicThread : Semaphore, Thread {
  ...
  public:
    PeriodicThread(Microsecond t, Function f, Argument a, Criterion c = Thread::Criterion::NORMAL)
        : Semaphore(0),
    ...
    Microsecond period() const { return m_period; }
    Microsecond period(Microsecond t) { m_period = t; }

    static void wait(Microsecond activation = 0) {
        PeriodicThread *self = static_cast<PeriodicThread *>(Thread::running());
        if (activation > 0) self->m_next = activation;
