   ...
   public:
    static void init() {
        ...
        if constexpr (Traits<System>::MULTITASK) {
            ...
            TraceIn(TextStart, TextSize, DataStart, DataSize, BssStart, BssSize);

            Task *task = new (Heap::SYSTEM) Task();
            //    Segment text = Segment(__mm.app.text.start, TextEnd - TextStart);
            //    task->attach(text);

            //    new (Heap::SYSTEM)
            //        Thread(reinterpret_cast<int (*)(void *)>(__mm.app.entry), 0, Thread::Criterion::NORMAL, task);
            //    TraceOut();
            new (Heap::SYSTEM) Thread(main, 0, Thread::Criterion::NORMAL, task);
        } else {
